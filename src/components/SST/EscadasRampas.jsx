...
const EscadasRampas = ({ handleBackToEscavacaoFundacao, handleNextPage, dadosIniciais = {} }) => {
  ...
  const [respostas, setRespostas] = useState(dadosIniciais || {
    protecoesGuardaCorpo: '',
    pisoForracaoAntiderrapante: '',
    dimensionadosCargas: '',
    sistemaProtecaoQuedas: '',
    escadasPortateisUltrapassam: ''
  });
  ...
  const handleAvancar = () => {
    if (isFormCompleto()) {
      ...
      const dados = {
        respostasEscadasRampas: respostas
      };
            ...
            >
              <CardContent>
                ...
                <Typography variant="body2" sx={{ mb: 3, color: 'text.secondary' }}>
                  Avalie as condições de escadas e rampas do canteiro de obras.
                  Estes elementos de circulação devem atender às normas de segurança
                  para prevenir quedas e outros acidentes.
                </Typography>
                ...
                <Stepper activeStep={11} orientation="vertical" sx={{ mb: 3 }}>
                  {STEPS.map((label) => (
                    <Step key={label}>
          ...
          <Card elevation={4} sx={{ borderRadius: 2, overflow: 'hidden' }}>
            ...
            <Box sx={{ 
              ...
              <Typography variant="subtitle1">
                Avaliação das condições de segurança em escadas e rampas no canteiro de obras
              </Typography>
            </Box>
            ...
            <Box sx={{ p: 4 }}>
              <Grid container spacing={1} sx={{ mb: 4 }}>
                <Grid item xs={12}>
                  ...
                  >
                    <StairsIcon sx={{ mr: 1 }} /> Escadas e Circulação
                  </Typography>
                  <Typography variant="body2" color="text.secondary" sx={{ mt: 1, mb: 3 }}>
                    Avalie se as escadas e rampas atendem às normas de segurança do trabalho.
                  </Typography>
                </Grid>
              </Grid>

              {/* Perguntas sobre escadas e rampas */}
              <Questao 
                numero="68"
                texto="As escadas e rampas possuem proteções constituído de guarda corpo, corrimão e tela?"
                name="protecoesGuardaCorpo"
                value={respostas.protecoesGuardaCorpo}
              />
              
              <Questao 
                numero="69"
                texto="Possui piso com forração completa e antiderrapante?"
                name="pisoForracaoAntiderrapante"
                value={respostas.pisoForracaoAntiderrapante}
              />
              
              <Questao 
                numero="70"
                texto="As escadas, rampas e passarelas são dimensionados e contruídos em função das cargas a que estarão submetidas?"
                name="dimensionadosCargas"
                value={respostas.dimensionadosCargas}
              />
              
              <Questao 
                numero="71"
                texto="As escadas fixas de uso coletivo são dotadas de sistema de proteção contra quedas?"
              ...
              <Questao 
                numero="72"
                texto="As escadas portáteis quando utilizada para acesso, são posicionados de forma a ultrapassar em pelo menos um 1m do nível superior?"
                name="escadasPortateisUltrapassam"
                value={respostas.escadasPortateisUltrapassam}
              />
                ...
                <Grid item xs={12}>
                  <Box sx={{ 
                    ...
                    <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap' }}>
                      <Button 
                        variant="outlined" 
                        onClick={handleBackToEscavacaoFundacao}
