...
const GuinchoColuna = ({ handleBackToElevadorCremalheira, handleNextPage, dadosIniciais = {} }) => {
  ...
  const [respostas, setRespostas] = useState(dadosIniciais || {
    operadorTreinado: '',
    linhaVidaOperador: '',
    capacidadeCargaInferior: '',
    analiseRiscoProcedimento: '',
    dispositivosFixacao: '',
    tamborNivelado: '',
    protecaoTambor: '',
    comandoEletrico: '',
    dispositivoPartidaParada: '',
  });
  ...
  const handleAvancar = () => {
    if (isFormCompleto()) {
      ...
      const dados = {
        respostasGuinchoColuna: respostas
      };
            ...
            >
              <CardContent>
                ...
                <Typography variant="body2" sx={{ mb: 3, color: 'text.secondary' }}>
                  Avalie as condições do guincho de coluna/mini grua no canteiro de obras.
                ...
                <Stepper activeStep={17} orientation="vertical" sx={{ mb: 3 }}>
                  {STEPS.map((label) => (
                    <Step key={label}>
          ...
          <Card elevation={4} sx={{ borderRadius: 2, overflow: 'hidden' }}>
            ...
            <Box sx={{ 
              ...
              <Typography variant="subtitle1">
                Avaliação das condições de segurança do guincho de coluna no canteiro de obras
              </Typography>
            </Box>
            ...
            <Box sx={{ p: 4 }}>
              <Grid container spacing={1} sx={{ mb: 4 }}>
                <Grid item xs={12}>
                  ...
                  >
                    <EngineeringIcon sx={{ mr: 1 }} /> Guincho de Coluna / Mini Grua
                  </Typography>
                  <Typography variant="body2" color="text.secondary" sx={{ mt: 1, mb: 3 }}>
                    Avalie se o guincho de coluna e a área de operação atendem aos requisitos de segurança estabelecidos nas normas.
                  </Typography>
                </Grid>
              </Grid>

              {/* Perguntas sobre guincho de coluna / mini grua */}
              <Questao 
                numero="121"
                texto="O operador está treinado para operação do guincho de coluna?"
              ...
              <Questao 
                numero="122"
                texto="Possui linha de vida para o operador?"
                name="linhaVidaOperador"
                value={respostas.linhaVidaOperador}
              />
              
              <Questao 
                numero="123"
                texto="Possui capacidade de carga não superior a 500 kg (quinhentos quilos)?"
                name="capacidadeCargaInferior"
                value={respostas.capacidadeCargaInferior}
              />
              
              <Questao 
                numero="124"
                texto="Possui análise de risco e procedimento operacional?"
                name="analiseRiscoProcedimento"
                value={respostas.analiseRiscoProcedimento}
              />
              
              <Questao 
                numero="125"
                texto="Possui dispositivos adequados para sua fixação, especificados no projeto de instalação?"
                name="dispositivosFixacao"
                value={respostas.dispositivosFixacao}
              />
              
              <Questao 
                numero="126"
                texto="Possui tambor nivelado para garantir o enrolamento adequado do cabo de aço?"
                name="tamborNivelado"
                value={respostas.tamborNivelado}
              />
              
              <Questao 
                numero="127"
                texto="Possui proteção para impedir o contato de qualquer parte do corpo do trabalhador com o tambor de enrolamento?"
                name="protecaoTambor"
                value={respostas.protecaoTambor}
              />
              
              <Questao 
                numero="128"
                texto="Possui comando elétrico por botoeira ou manipulador a cabo, respeitando voltagem máxima de 24V (vinte e quatro volts)?"
                name="comandoEletrico"
                value={respostas.comandoEletrico}
              />
              
              <Questao 
                numero="129"
                texto="O equipamento possui dispositivo de partida e parada com botão de emergência, aterramento, extintor de incêndio e checklist diário?"
                name="dispositivoPartidaParada"
                value={respostas.dispositivoPartidaParada}
              />
                ...
                <Grid item xs={12}>
                  <Box sx={{ 
                    ...
                    <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap' }}>
                      <Button 
                        variant="outlined" 
                        onClick={handleBackToElevadorCremalheira}
